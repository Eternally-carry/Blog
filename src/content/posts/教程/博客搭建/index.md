...
没有安装pnpm的,可以使用`npm install -g pnpm@latest-10`来安装
:::
...
:::important[注意]
如果不打算本地调试，未安装node.js可以跳过补全依赖   
直接去修改配置文件和编写博客文章的Makerdown   
未安装node.js的包管理器，无法执行下面的指令  
...
把上述html代码写入 **src/FooterConfig.html** 文件后，发现插入的位置在页脚部分的头部，个人比较喜欢把备案信息放在页面的最低端，所以还要更改页脚模块的位置。   
...
编写完成makerdown后，就可以开始运行构建，让Mizuki生成正确的网页文件了。
> [!TIP]
> 在开启运行构建前，请确保补全了包管理器的相关的依赖  
...
### 注册域名
有很多网络服务商都提供域名注册服务，国内提供商自然是 [阿里云](https://wanwang.aliyun.com/domain) 和 [腾讯云](https://cloud.tencent.com/product/domain) 。国外服务商推荐 [Dynadot](https://www.dynadot.com/zh) 和 [spaceship](https://www.spaceship.com/zh/domains/) ，可以不用提交实名信息。
